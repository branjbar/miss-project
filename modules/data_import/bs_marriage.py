...
while 1:
    ...
    person_id += 1
    query += """
            INSERT INTO `all_persons_2014` (`id`, `uuid`, `first_name`, `prefix`, `last_name`, `date_1`, 'place_1',
            `date_2`, `place_2`, `role`, `register_id`, `register_type`)
            VALUES (%d,"%s","%s","%s","%s","%s","%s", "%s", "%s", %d, %d, "%s");
            """ % (person_id, groom['uuid'], groom['first_name'], groom['prefix'], groom['last_name'], register['date'],
    ...
    query += """
            ...
            `date_2`, `place_2`, `role`, `register_id`, `register_type`)
            VALUES (%d,"%s","%s","%s","%s","%s","%s", "%s", "%s", %d, %d, "%s");
            """ % (person_id, bride['uuid'], bride['first_name'], bride['prefix'], bride['last_name'], register['date'],
    ...
    query += """
            ...
            """ % (person_id, father_groom['uuid'], father_groom['first_name'], father_groom['prefix'], father_groom['last_name'], register['date'], 2, doc_id, 'marriage')

    person_id += 1
    query += """
            INSERT INTO `all_persons_2014` (id, uuid, `first_name`, prefix, `last_name`, date_1, role, `register_id`, `register_type`)
            VALUES (%d,"%s","%s","%s","%s","%s", %d, %d, "%s");
            """ % (person_id, mother_groom['uuid'], mother_groom['first_name'], mother_groom['prefix'], mother_groom['last_name'], register['date'], 2, doc_id, 'marriage')

    person_id += 1
    query += """
            INSERT INTO `all_persons_2014` (id, uuid, `first_name`, prefix, `last_name`, date_1, role, `register_id`, `register_type`)
            VALUES (%d,"%s","%s","%s","%s","%s", %d, %d, "%s");
            """ % (person_id, father_bride['uuid'], father_bride['first_name'], father_bride['prefix'], father_bride['last_name'], register['date'], 3, doc_id, 'marriage')

    person_id += 1
    query += """
            INSERT INTO `all_persons_2014` (id, uuid, `first_name`, prefix, `last_name`, date_1, role, `register_id`, `register_type`)
            VALUES (%d,"%s","%s","%s","%s","%s", %d, %d, "%s");
            """ % (person_id, mother_bride['uuid'], mother_bride['first_name'], mother_bride['prefix'], mother_bride['last_name'], register['date'], 3, doc_id, 'marriage')
