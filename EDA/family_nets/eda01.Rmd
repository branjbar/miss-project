...
```{r}
network <- data.frame(no=integer(), child.max=integer(), parent.max=integer(), anomalies=integer(), depth=factor())
  ...
  for (i in 1:max(data[data$depth==d,]$network_no) ){
    ...
    if (nrow(net) > 1) {  
      ...
      instance <- data.frame(no=net$network_no[1], 
                             ...
                             anomalies=sum(parents$row.names>2),
                             depth=d)
...
###
ggplot(data=network, aes(x=parent.max,fill=depth)) + geom_histogram(position="dodge") + labs(x="Number of Parents", y="Freq.")
ggplot(data=network, aes(x=depth,y=parent.max)) + geom_boxplot() + labs(x="Depth",y="Number of Parents")

###
ggplot(data=network, aes(x=anomalies, fill=depth)) + geom_histogram(position="dodge") + labs(x="Number of Anomalies.",y="Freq.")
ggplot(data=network, aes(x=depth,y=anomalies)) + geom_boxplot() + labs(x="Depth", y="Number of Anomalies.")

###
ggplot(data=network, aes(x=anomalies==0,fill=depth)) + geom_histogram(position="dodge") + labs(x="Number of Anomalies", y="Freq.")
``` 
